"""
This module demonstrates how to model a simple linear time discrete system with pytorch by a recurrent neural
network (RNN) and identify the entries of the system matrix only from partial measurement. Because we do not
want to have any nonlinearity, we cannot use the rnn-class of pytorch, but instead build up a custom net
(see .forward(...) method).
"""
